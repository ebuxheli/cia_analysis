...
### year: year of data
### t: unknown
### active_d: indicator for whether the country was an active participant or transporter?? [not quite sure]
### active_t: indicator for whether the country was an active participant or transporter?? [not quite sure]
      ## In the pre-processing code, it is stated that "t" means treatment (this may imply that "d" means data and        ## "t" means treatment)
...
## In exploring the data, I found that it was difficult ot pinpoint the year in 
## which the country became an active participant in the RDI program specifically.
## It seems like there is some other factor that effects that data since it ranges
## from 1992 to 2011. Most countries have an active indicator in 1992, which would
## be impossible for the RDI program since the program only because active in 2001.
data_final %>%  
  filter(active_t == 1) %>% 
  glimpse() %>% 
...
```{r diff_means}
# This data was not used in the presentation of a figure in the paper.
