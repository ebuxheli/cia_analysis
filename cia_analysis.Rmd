...
# Figure 1
```{r fig1, cache=TRUE, include=TRUE}
...
graph_data <- gather(trends, type, value, var_names)  
graph_data$type <- factor(graph_data$type, levels = var_names)
...
var_limits <- as_tibble(cbind(year = c(rep(1992:2011, times = 4)), 
...
var_limits$type <- rep(var_names[1:4], times = 1, each = 20) 

# Converts labels to factors to order plot facets 
var_limits$type <- factor(var_limits$type, levels = var_names) 
...
# # Compute global-year averages for democracies and non-democracies for all var_names
...
#             "Disappearance Score (CIRI)" = mean(DISAP, na.rm = TRUE))
# 
# graph_data <- gather(trends, type, value, var_names) # Reformat data for graphs 
# graph_data$type <- factor(graph_data$type, levels = var_names) # Converts labels to factors to order plot facets 
# 
...
# graph1 <- subset(graph_data, (graph_data$type %in% var_names[1:4]))
# graph2 <- subset(graph_data, (graph_data$type %in% var_names[5:8]))
# 
...
# # Assign variable names
# facetlims1$type <- rep(var_names[1:4], times = 1, each = 40)
# facetlims2$type <- rep(var_names[5:8], times = 1, each = 40)
# # Converts labels to factors to order plot facets 
# facetlims1$type <- factor(facetlims1$type, levels = var_names) 
# facetlims2$type <- factor(facetlims2$type, levels = var_names)
# 
# # Create plots
# ## Figure 2
