...
```{r table23, results='asis', out.width='100%'}
############################################
...
mod_2 <- function(DV, LAG_DV){
  ...
  for (i in 1:imps){
    mod <- plm(fmla, data=data$imputations[[i]], index=c("COW", "YEAR"), effect ="twoways", model="within")
    mod <- coeftest(mod, vcovHC(mod, type = "HC3", cluster = "group"))
    betas[i,] <- mod[,1]
    st.errors[i,] <- mod[,2]
  }
  beta_estimates <- colMeans(betas)
  crse_estimates <- aggregate_se(betas, st.errors)
  mod <- plm(fmla, data=data$imputations[[1]], index=c("COW", "YEAR"), effect ="twoways", model="within")
...
footnote <- c("All models include country and year fixed effects and a dependent variable lagged one year. Country level",
